...
async fn test_key_at(url: &str, key: &str) -> Result<(), ProviderError> {
    ...
    if status.is_success() {
        ...
        Err(ProviderError::AuthFailed(format!("gemini returned {status}")))
    } else {
        Err(ProviderError::Unreachable(format!("gemini returned {status}")))
    }
}
...
impl GeminiProvider {
    pub fn new(api_key: String) -> Self {
        Self { api_key, client: reqwest::Client::new(), base_url: GEMINI_URL.to_string() }
    }
...
impl SummarizationProvider for GeminiProvider {
    ...
    async fn summarize(&self, transcript: &[TranscriptSegment]) -> Result<Summary, ProviderError> {
        ...
        if status == 401 || status == 403 {
            return Err(ProviderError::AuthFailed(format!("gemini returned {status}")));
        }
        if status == 429 {
        ...
        if !status.is_success() {
            return Err(ProviderError::Unreachable(format!("gemini returned {status}")));
        }
...
fn parse_response(body: &Value) -> Result<Summary, ProviderError> {
    let text = body
        ...
        .ok_or_else(|| ProviderError::BadResponse("missing candidates[0].content.parts[0].text".into()))?;
...
mod tests {
    ...
    async fn test_key_succeeds_on_2xx() {
        ...
        Mock::given(method("GET")).and(query_param("key", "good-key")).respond_with(ResponseTemplate::new(200)).mount(&mock_server).await;
    ...
    async fn test_key_reports_auth_failed_on_403() {
        let mock_server = MockServer::start().await;
        Mock::given(method("GET")).respond_with(ResponseTemplate::new(403)).mount(&mock_server).await;
