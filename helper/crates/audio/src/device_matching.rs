...
pub fn find_matching_device<'a>(available: &'a [String], hint: &str) -> Option<&'a str> {
    ...
    available.iter().find(|name| name.to_lowercase().contains(&hint_lower)).map(String::as_str)
}
...
mod tests {
    ...
    fn finds_device_case_insensitively() {
        let devices = vec!["Built-in Microphone".to_string(), "BlackHole 2ch".to_string()];
        assert_eq!(find_matching_device(&devices, MACOS_DEVICE_HINT), Some("BlackHole 2ch"));
    }

    #[test]
    fn returns_none_when_driver_not_installed() {
        let devices = vec!["Built-in Microphone".to_string(), "Built-in Output".to_string()];
    ...
    fn matches_vb_cable_output_device_name() {
        ...
        assert_eq!(find_matching_device(&devices, WINDOWS_DEVICE_HINT), Some("CABLE Output (VB-Audio Virtual Cable)"));
    }

    #[test]
    fn matches_first_hit_when_multiple_variants_present() {
        let devices = vec!["BlackHole 16ch".to_string(), "BlackHole 2ch".to_string()];
        assert_eq!(find_matching_device(&devices, MACOS_DEVICE_HINT), Some("BlackHole 16ch"));
    }
}
